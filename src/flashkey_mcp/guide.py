...
ERROR_GUIDE: dict[str, dict[str, Any]] = {
    E.DEVICE_NOT_FOUND: {
        ...
        "retryable": True,
        "recovery_tool": "flashkey_recover(reattach=True)",
    },
    E.HANDSHAKE_FAILED: {
    ...
    E.PORT_BUSY: {
        ...
        "retryable": True,
        "recovery_tool": "flashkey_status",
    },
    E.PORT_WRONG_ROLE: {
        "meaning": "用错端口角色",
        "hint": "先用 flashkey_list_ports() 按 role 字段选择 fk_log 端口，再重试",
        "retryable": True,
    ...
    E.FLASH_PROTECTED: {
        ...
        "retryable": True,
        "recovery_tool": "flashkey_firmware_flash",
    },
    E.FLASH_VERIFY_FAILED: {
        "meaning": "烧录校验不一致",
        "hint": "确认 chip 参数与固件匹配后重新烧录",
        "retryable": True,
        "recovery_tool": "flashkey_flash",
    },
    ...
    E.MODULE_MANIFEST_INVALID: {
        ...
        "retryable": True,
        "recovery_tool": "flashkey_module_info",
    },
    E.TIMEOUT: {
        "meaning": "响应超时",
...
1. 调用 `flashkey_status()` 确认 FK-01 已连接，并查看 `authed` 字段。
2. 调用 `flashkey_list_ports()` 获取串口列表，按 `role` 字段选择端口：
...
4. 烧录使用 `flash-firmware` prompt 或 `flashkey_flash()`；采集日志使用 `flashkey_log()`。
5. 出错时先读 `flashkey://docs/error-codes`，按 hint / recovery_tool 恢复
   （设备掉线通常先调 `flashkey_recover(reattach=True)`）。
...
FLASH_GUIDE_DOC = """\
# FlashKey 烧录指南（Ai-WB2 / Ai-M62）

## 端口选择（最重要）

- 必须选择 `flashkey_list_ports()` 中 `role=fk_log`（WCH-LinkE VCP）的端口。
...
| Ai-M62 | isp | 2000000 | `make flash`（SDK 内按 Ai-M62 对应芯片传 CHIP）：BOOT↑ + RST 进入 ISP |
...
1. `flashkey_list_ports()` → 记录 `role=fk_log` 的端口。
2. `flashkey_status()` → 未认证先完成密钥认证（SET_KEY / flashkey_auth）。
3. `flashkey_flash(firmware_path=<固件路径>, chip=<Ai-WB2/Ai-M62>, flash_port=<fk_log 端口>, baud_rate=<默认值>, ...)`。
4. 烧录后验证：`flashkey_log(port=<fk_log 端口>, baud_rate=<目标日志波特率>)` 观察启动日志；
   AT 模组可发送 `AT+GMR` 确认版本。

## 错误速查

- 错误码表：`flashkey://docs/error-codes`
- 设备掉线：`flashkey_recover(reattach=True)`
- 端口选错：重新 `flashkey_list_ports()` 选 `fk_log`
"""
...
_INSTRUCTIONS = (
    "FlashKey MCP：先读资源 quickstart、flash-guide。"
    "烧录/日志端口必须选 role=fk_log，禁用 role=fk_control，不要按端口名猜。"
    "先 list_ports 选端口，再 status 确认已认证，未认证先完成密钥认证。"
    "Ai-WB2 默认 break/921600，可 isp（make eflash，全量含 boot2）；"
    "Ai-M62 默认 isp/2000000。"
    "烧录后必须验证（flashkey_log 或 AT+GMR）。"
    "错误见 error-codes 资源；设备掉线先 recover(reattach=True)。"
)
...
def _prompt_flash_firmware(
    ...
    if chip_key == "bl602":
        ...
        chip_note = (
            f"{module_name} 使用 isp 模式，默认 2000000；sdk_path 指向 Bouffalo SDK。"
        )
    else:
        default_mode = "isp"
        default_baud = 2000000
    ...
    port_display = flash_port or "<由 flashkey_list_ports() 选择 role=fk_log 的端口>"
    chip_arg = (
        "ai-wb2"
        if chip_key == "bl602"
        else "ai-m62"
    ...
    if tool:
        ...
        sdk_display = sdk_path or "<sdk_path>"
        extra += (
    ...
    steps = (
        f"请按以下顺序执行烧录：\n"
        f"1. 调用 flashkey_list_ports()，找到 role=fk_log（WCH-LinkE VCP）的端口；"
        f"绝不要使用 role=fk_control，也不要按端口名猜测。\n"
        f"2. 调用 flashkey_status() 确认设备已连接且 authed=true；"
        f"未认证先完成密钥认证（SET_KEY / flashkey_auth）。\n"
        f"3. 调用 flashkey_flash(firmware_path=\"{firmware_path}\", chip=\"{chip_arg}\", "
        f"flash_port=\"{port_display}\", baud_rate={selected_baud}, mode=\"{selected_mode}\")"
        f"{extra}\n"
        f"   - {chip_note}\n"
        f"4. 烧录完成后验证：flashkey_log(port=\"{port_display}\", baud_rate=115200) "
        f"观察启动日志；AT 模组可发送 AT+GMR 确认版本。\n"
        f"5. 若返回错误码，先读 flashkey://docs/error-codes；"
        f"DEVICE_NOT_FOUND / HANDSHAKE_FAILED 时先 flashkey_recover(reattach=True)。"
    )
    return [
...
def _prompt_recover_device(error_code: str = "", context: str = "") -> list[Any]:
    ...
    if info:
        ...
        if code == E.DEVICE_NOT_FOUND or code == E.HANDSHAKE_FAILED:
            branch += (
                "决策：先调用 flashkey_recover(reattach=True) 重新挂载/握手；"
                "若仍失败，检查 WSL usbip attach 或 USB 物理连接。"
            )
        elif code == E.PORT_WRONG_ROLE:
            branch += (
                "决策：重新调用 flashkey_list_ports()，按 role 字段选择 fk_log"
        ...
        branch = (
            "未识别到具体错误码，按通用流程处理：\n"
            "1. 调用 flashkey_status() 检查设备连接与认证状态。\n"
            "2. 若设备掉线，调用 flashkey_recover(reattach=True)。\n"
...
def _prompt_collect_logs(
    ...
    """输出采集目标芯片日志的正确步骤。"""
    port_display = port or "<由 flashkey_list_ports() 选择 role=fk_log 的端口>"
    grep_args = f', grep="{grep}"' if grep else ""
    steps = (
        "请按以下顺序采集日志：\n"
        f"1. 调用 flashkey_list_ports()，选择 role=fk_log（WCH-LinkE VCP）的端口；"
        f"不要使用 role=fk_control。\n"
        f"2. 调用 flashkey_status() 确认设备已连接且 authed=true。\n"
        f"3. 调用 flashkey_log(port=\"{port_display}\", baud_rate={baud_rate}, "
        f"duration={duration}{grep_args})。\n"
        f"4. 注意：flashkey_log 与 flashkey_flash 互斥；若返回 PORT_BUSY，"
        f"等待当前烧录结束再重试。\n"
        f"5. 若设备掉线，先 flashkey_recover(reattach=True)。"
    )
    return [
        UserMessage(f"请采集目标芯片日志（{duration} 秒）"),
        AssistantMessage(steps),
    ]
...
def register_prompts(mcp: Any) -> None:
    ...
    mcp.add_prompt(
        Prompt(
            ...
            description=(
                ...
                "按芯片默认模式/波特率调用 flashkey_flash，最后验证。"
            ),
            arguments=[
    ...
    mcp.add_prompt(
        Prompt(
            name="collect-logs",
            title="采集目标芯片日志",
            description="输出采集日志的正确步骤：选 fk_log 端口、确认认证、调用 flashkey_log。",
            arguments=[
                PromptArgument(name="port", description="可选；role=fk_log 的串口"),
                PromptArgument(name="baud_rate", description="可选；日志波特率，默认 115200"),
                PromptArgument(name="duration", description="可选；采集秒数，默认 5"),
                PromptArgument(name="grep", description="可选；过滤关键词"),
            ],
